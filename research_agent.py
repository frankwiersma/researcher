...
def export_to_pdf(research_data: Dict[str, Any]) -> bytes:
    """
    Export research results to PDF format with proper markdown rendering.
    ...
    pdf.cell(0, 15, 'Research Report', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.ln(5)

    # Query section
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, 'Query', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_font('Arial', '', 11)
    pdf.multi_cell(0, 6, research_data['query'])
    pdf.ln(3)
    ...
    pdf.cell(0, 6, f"Generated: {timestamp.strftime('%Y-%m-%d %H:%M:%S')} | Model: {research_data['model']}",
    ...
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, 'Results', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(2)

    # Convert markdown to HTML
    result_text = research_data['result']
    ...
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 10, 'Generated by DeepAgents Research Assistant', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
